...
/*   Updated: 2024/05/16 12:43:59 by sel-bouy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	norm_putimages(t_data *i, int k)
{
	i->i = -1;
	while (i->map[i->j][++i->i])
	{
		...
		else if (i->map[i->j][i->i] == 'N')
			mlx_put_image_to_window(i->mx, i->m1, i->ens[k], \
			i->i * 64, i->j * 64);
	}
}
...
int	main(int ac, char **av)
{
	...
	|| img.co == NULL || img.pl == NULL || img.o_d == NULL || img.en == NULL)
	...
	mlx_loop_hook(img.mx, animation, &img);
	mlx_loop(img.mx);
}
